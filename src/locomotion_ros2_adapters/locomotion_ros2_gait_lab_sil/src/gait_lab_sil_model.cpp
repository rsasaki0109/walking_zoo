...
using AdapterStatus = locomotion_ros2_msgs::msg::AdapterStatus;
...
void GaitLabSilModel::configure(const locomotion_ros2_core::RobotProfile & profile)
{
  profile_ = profile;
...
locomotion_ros2_core::CallbackReturn GaitLabSilModel::activate()
{
  if (!configured_) {
...
locomotion_ros2_core::CallbackReturn GaitLabSilModel::deactivate()
{
  active_ = false;
...
locomotion_ros2_core::CallbackReturn GaitLabSilModel::cleanup()
{
  configured_ = false;
  active_ = false;
  have_sim_state_ = false;
...
locomotion_ros2_core::CommandResult GaitLabSilModel::command_velocity_gate(
  const geometry_msgs::msg::TwistStamped & cmd)
{
  if (!active_) {
    return locomotion_ros2_core::CommandResult::rejected("gait_lab SIL inactive");
  }
  if (estop_active_) {
    return locomotion_ros2_core::CommandResult::blocked("gait_lab SIL estopped");
  }
...
locomotion_ros2_core::CommandResult GaitLabSilModel::body_pose_gate()
{
  if (!active_) {
    return locomotion_ros2_core::CommandResult::rejected("gait_lab SIL inactive");
  }
...
locomotion_ros2_core::CommandResult GaitLabSilModel::footstep_gate()
{
  if (!active_) {
    return locomotion_ros2_core::CommandResult::rejected("gait_lab SIL inactive");
  }
...
locomotion_ros2_core::CommandResult GaitLabSilModel::stop_gate(locomotion_ros2_core::StopMode mode)
{
  (void)mode;
  if (!configured_) {
    return locomotion_ros2_core::CommandResult::rejected("gait_lab SIL unconfigured");
  }
  status_text_ = "gait_lab SIL stop forwarded";
  return locomotion_ros2_core::CommandResult::success(status_text_);
}

locomotion_ros2_core::CommandResult GaitLabSilModel::emergency_stop_gate()
{
  estop_active_ = true;
...
locomotion_ros2_core::CommandResult GaitLabSilModel::clear_fault_gate()
{
...
std::string GaitLabSilModel::control_for_stop(locomotion_ros2_core::StopMode mode) const
{
  switch (mode) {
    case locomotion_ros2_core::StopMode::EMERGENCY:
      return CTRL_ESTOP;
    case locomotion_ros2_core::StopMode::QUICK:
      return CTRL_STOP_QUICK;
    case locomotion_ros2_core::StopMode::NORMAL:
    default:
