...
using locomotion_ros2_gait_lab_sil::GaitLabSilModel;
using locomotion_ros2_core::StopMode;
...
{
  GaitLabSilModel m;
  m.configure(locomotion_ros2_core::RobotProfile{});
  return m;
}
...
TEST(GaitLabSilModel, VelocityRejectedUntilActive)
{
  ...
  ASSERT_EQ(m.activate(), locomotion_ros2_core::CallbackReturn::SUCCESS);
...
TEST(GaitLabSilModel, EstopBlocksThenClearFaultRecovers)
{
  ...
  EXPECT_EQ(blocked.status, locomotion_ros2_core::CommandStatus::BLOCKED);
