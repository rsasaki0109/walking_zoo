...
`locomotion_ros2_core::WalkingAdapter` contract and the dispatch-backend pattern
...
- `Sdk2SportBackend` (`src/sdk2_sport_backend.cpp`) is compiled **only** with
  `-DLOCOMOTION_ROS2_WITH_UNITREE_SDK2=ON`. It drives the Go2
...
```bash
colcon build --packages-select locomotion_ros2_unitree_go2 \
