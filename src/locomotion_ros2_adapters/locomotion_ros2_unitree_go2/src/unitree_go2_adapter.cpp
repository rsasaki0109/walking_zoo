#include "locomotion_ros2_unitree_go2/unitree_go2_adapter.hpp"
...
locomotion_ros2_core::CallbackReturn UnitreeGo2Adapter::configure(
  const locomotion_ros2_core::AdapterContext & context)
{
  profile_ = context.robot_profile;
  profile_.adapter_plugin = "locomotion_ros2_unitree_go2/UnitreeGo2Adapter";
...
locomotion_ros2_core::CallbackReturn UnitreeGo2Adapter::activate()
{
  if (!configured_) {
...
locomotion_ros2_core::CallbackReturn UnitreeGo2Adapter::deactivate()
{
  active_ = false;
  has_velocity_command_ = false;
...
locomotion_ros2_core::CallbackReturn UnitreeGo2Adapter::cleanup()
{
  configured_ = false;
  active_ = false;
  has_velocity_command_ = false;
...
locomotion_ros2_core::RobotProfile UnitreeGo2Adapter::get_robot_profile() const
{
  return profile_;
}

locomotion_ros2_msgs::msg::AdapterStatus UnitreeGo2Adapter::get_status() const
{
  ...
  status.adapter_name = "locomotion_ros2_unitree_go2/UnitreeGo2Adapter";
...
locomotion_ros2_msgs::msg::WalkingState UnitreeGo2Adapter::read_state()
{
  ...
  if (estop_active_) {
    ...
    switch (sport_mode_) {
      ...
      case SportMode::STAND_DOWN:
        ...
        state.locomotion_state = locomotion_ros2_msgs::msg::WalkingState::STATE_SITTING;
        state.locomotion_mode = locomotion_ros2_msgs::msg::WalkingState::MODE_IDLE;
        state.support_phase = locomotion_ros2_msgs::msg::WalkingState::SUPPORT_UNKNOWN;
        break;
  ...
  state.active_adapter = "locomotion_ros2_unitree_go2/UnitreeGo2Adapter";
  state.active_robot_model = profile_.robot_model;
  state.status_text = status_text_;
  return state;
}

locomotion_ros2_core::CommandResult UnitreeGo2Adapter::command_velocity(
  const geometry_msgs::msg::TwistStamped & cmd)
{
  if (!active_) {
    return locomotion_ros2_core::CommandResult::rejected("Unitree Go2 adapter inactive");
  }
  if (estop_active_) {
  ...
  if (sport.clamped) {
    return locomotion_ros2_core::CommandResult::limited("velocity clamped to Go2 envelope");
  }
  return locomotion_ros2_core::CommandResult::success(status_text_);
}

locomotion_ros2_core::CommandResult UnitreeGo2Adapter::command_body_pose(
  const locomotion_ros2_msgs::msg::BodyPoseCommand & cmd)
{
  ...
  if (posture.clamped) {
    return locomotion_ros2_core::CommandResult::limited("body pose clamped to Go2 posture envelope");
  }
  return locomotion_ros2_core::CommandResult::success(status_text_);
}

locomotion_ros2_core::CommandResult UnitreeGo2Adapter::execute_footstep_plan(
...
locomotion_ros2_core::CommandResult UnitreeGo2Adapter::stop(locomotion_ros2_core::StopMode mode)
{
  if (!configured_) {
    return locomotion_ros2_core::CommandResult::rejected("Unitree Go2 adapter unconfigured");
  }
  has_velocity_command_ = false;
  if (mode == locomotion_ros2_core::StopMode::EMERGENCY) {
    enter_mode(SportMode::DAMP);
    status_text_ = "Unitree Go2 emergency stop (damp)";
  } else if (mode == locomotion_ros2_core::StopMode::QUICK) {
...
locomotion_ros2_core::CommandResult UnitreeGo2Adapter::emergency_stop()
{
  estop_active_ = true;
  has_velocity_command_ = false;
  enter_mode(SportMode::DAMP);
...
locomotion_ros2_core::CommandResult UnitreeGo2Adapter::clear_fault()
{
  if (estop_active_) {
...
PLUGINLIB_EXPORT_CLASS(
  locomotion_ros2_unitree_go2::UnitreeGo2Adapter,
