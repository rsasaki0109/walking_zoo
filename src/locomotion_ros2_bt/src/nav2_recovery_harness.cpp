...
// Harness that exercises the locomotion_ros2 Nav2 recovery nodes through the *real*
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<rclcpp::Node>("locomotion_ros2_nav2_recovery_harness");

  std::string default_xml;
  try {
    default_xml = ament_index_cpp::get_package_share_directory("locomotion_ros2_bt") +
      "/bt_xml/locomotion_ros2_nav2_recovery_branch.xml";
  ...
  try {
    ...
    nav2_behavior_tree::BehaviorTreeEngine engine({"locomotion_ros2_nav2_bt_nodes"}, node);
