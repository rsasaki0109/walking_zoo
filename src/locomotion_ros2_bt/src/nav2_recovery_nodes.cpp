#include "locomotion_ros2_bt/nav2_recovery_nodes.hpp"
...
: BT::ConditionNode(name, conf)
{
  ...
  state_topic_ = "/locomotion_ros2/state";
...
BT::PortsList IsWalkingReadyCondition::providedPorts()
{
  return {
    BT::InputPort<std::string>(
      "state_topic", "/locomotion_ros2/state", "WalkingState topic to monitor")
  };
}

void IsWalkingReadyCondition::stateCallback(locomotion_ros2_msgs::msg::WalkingState::SharedPtr msg)
{
...
ClearWalkingFaultBtNode::ClearWalkingFaultBtNode(
  const std::string & name, const BT::NodeConfiguration & conf)
: nav2_behavior_tree::BtServiceNode<locomotion_ros2_msgs::srv::ClearFault>(
    name, conf, "/locomotion_ros2/clear_fault")
{
}
...
// `locomotion_ros2_nav2_bt_nodes` in the bt_navigator `plugin_lib_names` parameter is
// enough for Nav2 to register IsWalkingReady and ClearWalkingFault into the tree.
BT_REGISTER_NODES(factory)
{
  factory.registerNodeType<locomotion_ros2_bt::IsWalkingReadyCondition>("IsWalkingReady");
  factory.registerNodeType<locomotion_ros2_bt::ClearWalkingFaultBtNode>("ClearWalkingFault");
}
