...
: rclcpp::Node("locomotion_ros2_cmd_vel_bridge", options),
  last_cmd_time_(0, 0, RCL_ROS_TIME)
{
  const auto input_topic = declare_parameter<std::string>("input_topic", "/cmd_vel");
  const auto output_topic = declare_parameter<std::string>("output_topic", "/locomotion_ros2/cmd_vel");
  const auto state_topic = declare_parameter<std::string>("state_topic", "/locomotion_ros2/state");
  ...
  if (require_ready_) {
    state_sub_ = create_subscription<locomotion_ros2_msgs::msg::WalkingState>(
      state_topic,
      rclcpp::SystemDefaultsQoS(),
...
void CmdVelBridge::handle_state(const locomotion_ros2_msgs::msg::WalkingState::SharedPtr msg)
{
  got_state_ = true;
