#include "locomotion_ros2_runtime/walking_runtime_manager.hpp"
...
: rclcpp_lifecycle::LifecycleNode("locomotion_ros2_runtime_manager", options),
...
void WalkingRuntimeManager::declare_parameters()
{
  ...
  declare_parameter<std::string>("adapter_plugin", "locomotion_ros2_mock_adapter/MockWalkingAdapter");
...
locomotion_ros2_core::RobotProfile WalkingRuntimeManager::profile_from_parameters()
{
  ...
  if (!robot_profile_path.empty() && robot_profile_path != "mock") {
    profile = locomotion_ros2_core::load_robot_profile_from_yaml(robot_profile_path, profile);
  }
  ...
  const rclcpp_lifecycle::State & state)
{
  ...
  state_pub_ = create_publisher<locomotion_ros2_msgs::msg::WalkingState>(
    "/locomotion_ros2/state", rclcpp::SystemDefaultsQoS());
  adapter_status_pub_ = create_publisher<locomotion_ros2_msgs::msg::AdapterStatus>(
    "/locomotion_ros2/adapter_status", rclcpp::SystemDefaultsQoS());
  safety_state_pub_ = create_publisher<locomotion_ros2_msgs::msg::SafetyState>(
    "/locomotion_ros2/safety_state", rclcpp::SystemDefaultsQoS());
  ...
  estop_srv_ = create_service<locomotion_ros2_msgs::srv::EmergencyStop>(
    "/locomotion_ros2/estop",
    std::bind(
      &WalkingRuntimeManager::handle_estop,
      this,
      std::placeholders::_1,
      std::placeholders::_2));
  clear_fault_srv_ = create_service<locomotion_ros2_msgs::srv::ClearFault>(
    ...
    std::bind(
      ...
      std::placeholders::_1,
      std::placeholders::_2));
  set_mode_srv_ = create_service<locomotion_ros2_msgs::srv::SetLocomotionMode>(
    "/locomotion_ros2/set_locomotion_mode",
    std::bind(
  ...
  execute_velocity_server_ = rclcpp_action::create_server<ExecuteVelocity>(
    ...
    "/locomotion_ros2/execute_velocity",
    std::bind(
  ...
  execute_footstep_server_ = rclcpp_action::create_server<ExecuteFootstepPlan>(
    ...
    get_node_logging_interface(),
    get_node_waitables_interface(),
    "/locomotion_ros2/execute_footstep_plan",
    std::bind(
  ...
  execute_body_pose_server_ = rclcpp_action::create_server<ExecuteBodyPose>(
    ...
    get_node_logging_interface(),
    get_node_waitables_interface(),
    "/locomotion_ros2/execute_body_pose",
    std::bind(
  ...
  locomotion_ros2_core::AdapterContext context(get_logger(), get_clock());
  context.robot_profile = robot_profile_;
  context.allow_motion = allow_motion_;
  context.robot_profile_path = robot_profile_path_;
  if (adapter_->configure(context) != locomotion_ros2_core::CallbackReturn::SUCCESS) {
  ...
  RCLCPP_INFO(
    get_logger(),
    "Configured locomotion_ros2 runtime with adapter '%s' (allow_motion=%s)",
  ...
  const rclcpp_lifecycle::State & state)
{
  ...
  if (adapter_ && adapter_->activate() != locomotion_ros2_core::CallbackReturn::SUCCESS) {
    RCLCPP_ERROR(get_logger(), "Walking adapter activate failed");
    return LifecycleCallbackReturn::FAILURE;
  }
  publish_state();
  RCLCPP_INFO(get_logger(), "locomotion_ros2 runtime active");
  return LifecycleCallbackReturn::SUCCESS;
}
  ...
  const rclcpp_lifecycle::State & state)
{
  (void)state;
  if (adapter_) {
    adapter_->stop(locomotion_ros2_core::StopMode::NORMAL);
    adapter_->deactivate();
  }
  ...
  const rclcpp_lifecycle::State & state)
{
  (void)state;
  if (adapter_) {
    adapter_->stop(locomotion_ros2_core::StopMode::EMERGENCY);
  }
  return LifecycleCallbackReturn::SUCCESS;
}
  ...
  const geometry_msgs::msg::TwistStamped::SharedPtr msg)
{
  ...
  mode_manager_.set_mode(locomotion_ros2_msgs::msg::WalkingState::MODE_WALK);
  last_state_ = adapter_->read_state();
  if (!adapter_result.accepted) {
    last_state_.status_text = adapter_result.message;
  } else if (safety_result.result.status == locomotion_ros2_core::CommandStatus::LIMITED) {
    last_state_.status_text = safety_result.result.message;
  }
  publish_state();
}
...
void WalkingRuntimeManager::publish_state()
{
  ...
  if (safety_state_pub_ && safety_state_pub_->is_activated()) {
    ...
    if (state.is_fallen && !safety_state.estop_active) {
      safety_state.state = locomotion_ros2_msgs::msg::SafetyState::STATE_FAULT;
  ...
  const std::shared_ptr<GoalHandleExecuteVelocity> goal_handle)
{
  (void)goal_handle;
  if (adapter_) {
    adapter_->stop(locomotion_ros2_core::StopMode::QUICK);
  }
  ...
  const std::shared_ptr<GoalHandleExecuteFootstepPlan> goal_handle)
{
  ...
  mode_manager_.set_mode(locomotion_ros2_msgs::msg::WalkingState::MODE_FOOTSTEP);
  ...
  const std::shared_ptr<GoalHandleExecuteBodyPose> goal_handle)
{
  ...
  mode_manager_.set_mode(locomotion_ros2_msgs::msg::WalkingState::MODE_BODY_POSE);
  ...
  result->success = true;
  result->status_text = safety.result.status == locomotion_ros2_core::CommandStatus::LIMITED ?
