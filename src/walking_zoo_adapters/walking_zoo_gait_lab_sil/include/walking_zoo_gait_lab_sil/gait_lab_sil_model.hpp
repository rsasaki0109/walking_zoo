#ifndef WALKING_ZOO_GAIT_LAB_SIL__GAIT_LAB_SIL_MODEL_HPP_
#define WALKING_ZOO_GAIT_LAB_SIL__GAIT_LAB_SIL_MODEL_HPP_
...
public:
  ...
  void configure(const walking_zoo_core::RobotProfile & profile);
  walking_zoo_core::CallbackReturn activate();
  walking_zoo_core::CallbackReturn deactivate();
  walking_zoo_core::CallbackReturn cleanup();
  ...
  walking_zoo_core::CommandResult command_velocity_gate(
    const geometry_msgs::msg::TwistStamped & cmd);
  walking_zoo_core::CommandResult body_pose_gate();
  walking_zoo_core::CommandResult footstep_gate();
  walking_zoo_core::CommandResult stop_gate(walking_zoo_core::StopMode mode);
  walking_zoo_core::CommandResult emergency_stop_gate();
  walking_zoo_core::CommandResult clear_fault_gate();

  // The control string to forward for a lifecycle/stop transition (empty = none).
  std::string control_for_stop(walking_zoo_core::StopMode mode) const;

  // Fuse a WalkingState reported by the sim, timestamped `now_sec`.
  void ingest_sim_state(const walking_zoo_msgs::msg::WalkingState & state, double now_sec);
  ...
  walking_zoo_msgs::msg::WalkingState read_state(double now_sec) const;
  walking_zoo_msgs::msg::AdapterStatus get_status(double now_sec) const;
  const walking_zoo_core::RobotProfile & profile() const {return profile_;}
...
private:
  ...
  walking_zoo_msgs::msg::WalkingState sim_state_;
  double freshness_timeout_sec_{0.5};
};

}  // namespace walking_zoo_gait_lab_sil

#endif  // WALKING_ZOO_GAIT_LAB_SIL__GAIT_LAB_SIL_MODEL_HPP_
