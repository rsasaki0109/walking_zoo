...
(`include/walking_zoo_unitree_sdk2/loco_backend.hpp`). Everything above it —
...
- `Sdk2LocoBackend` (`src/sdk2_loco_backend.cpp`, compiled only with
  `-DWALKING_ZOO_WITH_UNITREE_SDK2=ON`): initialises the DDS channel and the G1
...
```bash
colcon build --packages-select walking_zoo_unitree_sdk2 \
...
appear in `walking_zoo_core` or `walking_zoo_msgs`; `src/sdk2_loco_backend.cpp`
