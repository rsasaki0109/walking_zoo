...
// A live walking_zoo recovery node. It subscribes to /walking_zoo/state, ticks
// the recovery behavior tree, and lets the ClearWalkingFaultService BT node call
// /walking_zoo/clear_fault when the robot is not ready. This is the missing wire
...
class WalkingZooRecoveryNode
{
public:
  explicit WalkingZooRecoveryNode(const rclcpp::Node::SharedPtr & node)
  : node_(node)
  {
    std::string default_xml;
    try {
      default_xml = ament_index_cpp::get_package_share_directory("walking_zoo_bt") +
        "/bt_xml/walking_zoo_recovery_live.xml";
    } catch (const std::exception &) {
      default_xml = "";
    }
    const std::string xml_path = node_->declare_parameter<std::string>("bt_xml_path", default_xml);
    const std::string state_topic =
      node_->declare_parameter<std::string>("state_topic", "/walking_zoo/state");
    const std::string clear_fault_service =
      node_->declare_parameter<std::string>("clear_fault_service", "/walking_zoo/clear_fault");
    ...
    context_ = std::make_shared<walking_zoo_bt::RecoveryContext>();
    context_->node = node_;
    context_->clear_fault_client =
      node_->create_client<walking_zoo_msgs::srv::ClearFault>(clear_fault_service);
    context_->service_timeout = std::chrono::duration<double>(service_timeout);

    BT::BehaviorTreeFactory factory;
    walking_zoo_bt::register_recovery_bt_nodes(factory, context_);
    ...
    RCLCPP_INFO(
      node_->get_logger(),
      "walking_zoo recovery node ticking '%s' every %.2fs", xml_path.c_str(), tick_period_);
  }
  ...
  std::optional<BT::NodeStatus> tick_once()
  {
    ...
    if (status == BT::NodeStatus::SUCCESS && last_status_ != BT::NodeStatus::SUCCESS) {
      RCLCPP_INFO(node_->get_logger(), "walking_zoo recovery: robot is ready");
    } else if (status == BT::NodeStatus::FAILURE && last_status_ != BT::NodeStatus::FAILURE) {
      RCLCPP_WARN(node_->get_logger(), "walking_zoo recovery: robot not ready, recovering");
    }
    last_status_ = status;
    return status;
  }
...
private:
  ...
  std::shared_ptr<walking_zoo_bt::RecoveryContext> context_;
  BT::Tree tree_;
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<rclcpp::Node>("walking_zoo_bt_recovery");
  auto recovery = std::make_shared<WalkingZooRecoveryNode>(node);
