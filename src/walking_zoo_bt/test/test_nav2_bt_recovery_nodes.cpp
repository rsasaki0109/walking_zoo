// ROS-integrated tests for the Nav2-loadable walking_zoo recovery nodes. The
...
// driven against a fake /walking_zoo/clear_fault service plus a /walking_zoo/state
...
using walking_zoo_msgs::srv::ClearFault;
  ...
  <BehaviorTree ID="WalkingRecoveryBranch">
    <Sequence name="WalkingFaultRecovery">
      <Fallback name="ReadyOrClear">
        <IsWalkingReady state_topic="/walking_zoo/state"/>
        <ClearWalkingFault service_name="/walking_zoo/clear_fault" reason="nav2_bt_recovery"/>
      </Fallback>
      <IsWalkingReady state_topic="/walking_zoo/state"/>
    </Sequence>
  </BehaviorTree>
...
// /walking_zoo/clear_fault. When configured to recover, a successful clear flips
...
public:
  ...
  {
    state_pub_ = node_->create_publisher<WalkingState>("/walking_zoo/state", 10);
...
protected:
  ...
  BT::Tree make_tree()
  {
    ...
    factory.registerFromPlugin(WALKING_ZOO_NAV2_BT_NODES_LIB);
