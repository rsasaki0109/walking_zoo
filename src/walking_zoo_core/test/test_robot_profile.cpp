...
{
  const auto path = std::filesystem::temp_directory_path() / "walking_zoo_robot_profile_test.yaml";
...
safety:
  ...
  const auto profile = walking_zoo_core::load_robot_profile_from_yaml(path.string());

  EXPECT_EQ(profile.robot_model, "unitree_g1");
  EXPECT_EQ(profile.robot_family, "humanoid");
  EXPECT_EQ(profile.adapter_plugin, "walking_zoo_unitree_sdk2/UnitreeSdk2Adapter");
