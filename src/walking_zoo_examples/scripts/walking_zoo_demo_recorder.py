#!/usr/bin/env python3
"""Record a compact runtime trace for walking_zoo demos."""
...
class WalkingZooDemoRecorder(Node):
    def __init__(self):
        super().__init__("walking_zoo_demo_recorder")
        self.declare_parameter("output_dir", "/tmp/walking_zoo_mujoco_g1_showcase")
        ...
        self.create_subscription(TwistStamped, "/walking_zoo/cmd_vel", self.on_walking_zoo_cmd_vel, 10)
        self.create_subscription(SemanticAction, "/walking_zoo/semantic_action", self.on_semantic_action, 10)

        period = float(self.get_parameter("write_period_sec").value)
        self.timer = self.create_timer(max(period, 0.2), self.write_trace)
        self.get_logger().info(f"recording walking_zoo demo trace to {self.output_dir}")
    ...
    def on_walking_state(self, msg):
        ...
        if state != self.last_state:
            ...
            self.append_event("/walking_zoo/state", f"walking state -> {state}", data)
    ...
    def on_adapter_status(self, msg):
        ...
        if key != self.last_adapter:
            ...
            self.append_event("/walking_zoo/adapter_status", f"adapter -> {status}", data)
    ...
    def on_safety_state(self, msg):
        ...
        if key != self.last_safety:
            ...
            self.append_event("/walking_zoo/safety_state", f"safety -> {state}", data)
    ...
    def on_walking_zoo_cmd_vel(self, msg):
        data = self.twist_data(msg.twist)
        data["frame_id"] = msg.header.frame_id
        self.latest["walking_zoo_cmd_vel"] = data
        self.append_event("/walking_zoo/cmd_vel", self.twist_summary(data), data)
    ...
    def on_semantic_action(self, msg):
        ...
        self.append_event("/walking_zoo/semantic_action", f"semantic -> {msg.action}", data)
