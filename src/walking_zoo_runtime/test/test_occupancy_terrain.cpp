...
using walking_zoo_runtime::FootstepPlanner;
using walking_zoo_runtime::FootstepPlanParams;
...
TEST(OccupancyTerrain, CostmapGeometryIsCopied)
{
  ...
  const auto grid = walking_zoo_runtime::terrain_grid_from_costmap(costmap, 50);
...
TEST(OccupancyTerrain, CostmapDrivesKeepOut)
{
  ...
  terrain.set_grid(walking_zoo_runtime::terrain_grid_from_costmap(costmap, 50));
...
TEST(OccupancyTerrain, ThresholdControlsWhatCountsAsBlocked)
{
  ...
  costmap.data[0] = 60;
  TerrainModel strict;
  strict.set_grid(walking_zoo_runtime::terrain_grid_from_costmap(costmap, 50));
  EXPECT_TRUE(strict.is_no_step(0.2, 0.2));

  TerrainModel lenient;
  lenient.set_grid(walking_zoo_runtime::terrain_grid_from_costmap(costmap, 80));
...
{
  auto costmap = make_costmap(2, 1, 0.5, 0.0, 0.0, -1);
  auto grid = walking_zoo_runtime::terrain_grid_from_costmap(costmap, 50);

  auto elevation = make_costmap(2, 1, 0.5, 0.0, 0.0, -1);
  elevation.data[1] = 80;  // 80 * 0.0015 m = 0.12 m
  ASSERT_TRUE(walking_zoo_runtime::set_elevation_from_grid(grid, elevation, 0.0015));
...
{
  auto grid = walking_zoo_runtime::terrain_grid_from_costmap(
    make_costmap(2, 1, 0.5, 0.0, 0.0, -1), 50);
  auto wrong = make_costmap(3, 1, 0.5, 0.0, 0.0, -1);
  EXPECT_FALSE(walking_zoo_runtime::set_elevation_from_grid(grid, wrong, 0.0015));
...
TEST(OccupancyTerrain, PlannerDodgesCostmapObstacle)
{
  ...
  EXPECT_EQ(result.plan.planner_id, "walking_zoo_terrain_planner");
