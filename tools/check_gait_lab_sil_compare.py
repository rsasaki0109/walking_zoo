...
def run_one(rclpy, controller, env, sim_python, speed, horizon, connect_timeout):
    ...
    return a result dict. ``status`` is ok / FELL / stand / skipped."""
...
def main() -> int:
    ...
    print("gait_lab controllers through the locomotion_ros2 runtime "
          f"(cmd {args.speed} m/s, horizon {args.horizon}s):")
    ...
    md = ["# gait_lab controllers through the locomotion_ros2 runtime", "",
          f"Command `{args.speed} m/s` forward, horizon `{args.horizon}s`, scored "
          "from `/locomotion_ros2/state` and the sim's base odometry — every gait "
          "driven through the real runtime + safety pipeline.", "",
