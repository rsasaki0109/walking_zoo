...
class G1ShowcaseScene:
    ...
    def forward_gait(self, qpos, sin_phase, cos_phase, fast=False):
        ...
        self.set_joint(qpos, "waist_pitch_joint", -0.12 if fast else -0.07)
        self.set_joint(qpos, "waist_yaw_joint", (0.04 if fast else 0.05) * sin_phase)
        self.set_joint(qpos, "waist_roll_joint", 0.02 * cos_phase if fast else 0.0)
        ...
        self.set_joint(qpos, "left_shoulder_roll_joint", 0.12 if fast else 0.10)
        self.set_joint(qpos, "right_shoulder_roll_joint", -0.12 if fast else -0.10)
    ...
    def pose(self, frame_index, gait):
        ...
        if gait == "stand":
            ...
            qpos[3:7] = self.body_quat(0.0, -0.11 + 0.006 * sin_phase, 0.0)
